...
> 버전 0.6.0 · Windows 11 / 폐쇄망 친화 · 독립 신규 프로젝트 (`prism` 자매 프로젝트)
> 백엔드(FastAPI) + 단일 HTML UI 구현 완료 · PyInstaller onedir 배포 가능
> v0.6.0: 검색 정확도 튜닝(**문장경계 청크**, 인덱스 schema 4) + 검색 결과 클릭 시 **PDF 원문 매칭어 형광 표시**(비교조회·RAG 인라인 미리보기)
> v0.5.0: 재무제표 비교에 **연간 확정(FY)** 재무수치 + **다년 시계열** 보강(사업보고서 기준 `fnlttSinglAcntAll` 재무데이터만 — 본문 PDF·XBRL 없음)
> v0.4.0: 사업보고서(본문)·XBRL 태깅 기능 제거 — 검토보고서(연결/별도) 주석 검색 중심으로 축소
...
2. 라이브러리 매트릭스에 **연결·별도 검토보고서 둘 다** 수집 (연결/별도가 1급 차원)
3. 회계담당자용 **비교분석 기능** (시계열 증감·연결vs별도 차이·4사 벤치마킹·구조비율 등)

## 절대 안전경계 (불변)
- **AI 숫자 재구성 금지** — DART 원문 그대로
...
- ✅ **백엔드 (FastAPI)**: `src/app.py` 외 모듈
  - DART 자동수집(연결/별도 검토보고서) · 라이브러리 인덱싱 · 재무제표 결정론 비교(provenance)
  - 주석 RAG 검색(임베딩+BM25+동의어, 인용 강제) · 주제 매핑 · 비교메모(LLM 옵트인)
- ✅ **단일 HTML UI**: `src/static/index.html` (오프라인·외부 CDN 0, vendored pdf.js) — 5개 탭(라이브러리·비교조회·자연어질의·재무제표·커버리지)
...
- ✅ **테스트**: `tests/` — 단위 + Playwright E2E
- ✅ **배포**: PyInstaller onedir 풀번들 (`prism_fs.spec` + `build.ps1` + `run_server.py`)
- ✅ **오프라인 큐레이션 도구**(개발 PC 전용): `scripts/` — 인덱스 큐레이션·검색 평가
